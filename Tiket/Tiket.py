class Tiket:
    BATAS_BAGASI_GRATIS = 30
    BIAYA_KELEBIHAN_BAGASI = 50_000
    DISKON_SEMUA_DEWASA = 0.03
    DISKON_ADA_ANAK     = 0.02
    MIN_PENUMPANG_DISKON = 3
    ...
    def struk(self):
        ...
        lines = [
            ...
            f"Subtotal Tarif   : Rp {self.sebelum_diskon:,.0f}",   
        ]
        ...
        lines += [
            f"Tarif setelah diskon : Rp {self.setelah_diskon:,.0f}",  
            "-" * 50,
            f"Berat Bagasi     : {self.berat_bagasi} kg",
        ]
 
        if self.biayaBagasi() > 0:                                 
            kelebihan = self.berat_bagasi - self.BATAS_BAGASI_GRATIS
            lines.append(f"Biaya Bagasi     : {kelebihan} kg x Rp {self.BIAYA_KELEBIHAN_BAGASI:,} = Rp {self.biayaBagasi():,.0f}")  # fix: biayaBagasi()
        else:
            lines.append("Biaya Bagasi     : Gratis (≤ 30 kg)")
 
        lines += [
            "=" * 50,
            f"TOTAL HARGA      : Rp {self.kalkulasiTotal():,.0f}",  
            "=" * 50,
        ]
        return "\n".join(lines)
